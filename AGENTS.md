...
- Bazel VM tests use `//tests/vm:coregate_guest`, which wraps `//:coregate_guest` and disables SQLite to avoid linking bundled SQLite with the host C toolchain in a musl Rust binary. Use the normal `//:coregate` target for host/default SQLite builds.
