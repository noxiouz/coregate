...
fn run() -> Result<()> {
    let control_path = env::var("COREGATE_CONTROL_PORT").unwrap_or_else(|_| DEFAULT_CONTROL_PORT.to_string());
        ...
        match fs::OpenOptions::new().read(true).write(true).open(path) {
            ...
            Err(err) => {
                ...
                if Instant::now() > deadline {
                    return Err(err).with_context(|| format!("open control port {}", path.display()));
                }
...
fn run_scenario(
    ...
    if matches!(ingress_mode, VmIngressMode::Server | VmIngressMode::ServerLegacy) {
...
fn start_server_mode(mode: VmIngressMode) -> Result<Child> {
    ...
    if let Some(status) = child.try_wait().context("poll coregate serve")? {
        ...
        bail!(
            "coregate serve exited early with status {status}\nlog:\n{log}"
        );
    }
    Ok(child)
}
