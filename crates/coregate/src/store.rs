//! Persistent crash-record sinks.
//!
//! JSONL is always available. SQLite support is feature-gated here so callers
//! do not need to carry `#[cfg(feature = "sqlite")]` at every write site.

use crate::bpf::StackRecord;
use crate::corefile::CoreWriteResult;
...
/// Writes a SQLite record when a path is configured.
///
/// In builds without the `sqlite` feature this is a no-op with a diagnostic,
/// preserving one call path from the collector hot path.
pub fn insert_sqlite_if_configured(path: Option<&Path>, record: &CrashRecord) -> Result<()> {
    let Some(path) = path else {
        return Ok(());
    };

    insert_sqlite_impl(path, record)
}

#[cfg(feature = "sqlite")]
fn insert_sqlite_impl(path: &Path, record: &CrashRecord) -> Result<()> {
    if let Some(parent) = path.parent() {
    ...
    conn.execute(
        ...
        params![
            ...
            metadata.process_start_time_ms.map(|v| v as i64),
            metadata.process_uptime_ms.map(|v| v as i64),
            metadata.cgroup,
            metadata.binary_removed.map(|v| v as i64),
            metadata.dumpable.map(|v| v as i64),
            metadata.package_version,
            metadata.symbolization_mode,
            metadata.symbolization_status,
            stored,
            core_location,
            core_size,
...
fn insert_sqlite_impl(_path: &Path, _record: &CrashRecord) -> Result<()> {
...
fn ensure_schema(conn: &Connection) -> rusqlite::Result<()> {
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS crash_records (
            ...
            process_start_time_ms INTEGER,
            process_uptime_ms INTEGER,
            cgroup_name TEXT,
            binary_removed INTEGER,
            dumpable INTEGER,
            package_version TEXT,
            symbolization_mode TEXT,
            symbolization_status TEXT,
    ...
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN thread_name TEXT",
    )?;
    ...
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN binary_name TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN binary_path TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN binary_build_id TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN process_start_time_ms INTEGER",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN process_uptime_ms INTEGER",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN cgroup_name TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN binary_removed INTEGER",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN dumpable INTEGER",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN package_version TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN symbolization_mode TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN symbolization_status TEXT",
    )?;
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN core_stored INTEGER",
    )?;
    ensure_optional_column(
        conn,
    ...
    ensure_optional_column(
        conn,
        "ALTER TABLE crash_records ADD COLUMN core_size INTEGER",
    )?;
    ensure_optional_column(
        conn,
