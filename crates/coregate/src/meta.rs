...
pub struct CrashMetadata {
    ...
    pub symbolization_status: Option<String>,
}
...
pub fn collect_basic(pid: i32, tid: Option<i32>) -> Result<CrashMetadata> {
    ...
    let system_uptime_ms = read_uptime_ms().ok();
    let process_timing = read_process_timing(pid, system_uptime_ms).ok();
    ...
    Ok(CrashMetadata {
        ...
        process_start_time_ms: process_timing.as_ref().map(|timing| timing.start_time_ms),
        process_uptime_ms: process_timing.map(|timing| timing.process_uptime_ms),
...
fn read_process_timing(pid: i32, system_uptime_ms: Option<u64>) -> Result<ProcessTiming> {
    let raw = fs::read_to_string(format!("/proc/{pid}/stat")).context("read /proc/<pid>/stat")?;
    let (_, fields) = split_stat_fields(&raw).context("split stat fields")?;
    let start_ticks = fields
        .get(19)
        .context("missing starttime field in stat")?
        .parse::<u64>()
        .context("parse starttime field")?;
    let clock_ticks = clock_ticks_per_second()?;
    let start_since_boot_ms = start_ticks.saturating_mul(1_000) / clock_ticks;
    let boot_time_ms = read_boot_time_ms()?;
    let process_uptime_ms = system_uptime_ms
        .unwrap_or_else(|| read_uptime_ms().unwrap_or(0))
        .saturating_sub(start_since_boot_ms);

    Ok(ProcessTiming {
        start_time_ms: boot_time_ms.saturating_add(start_since_boot_ms),
        process_uptime_ms,
    })
}

fn clock_ticks_per_second() -> Result<u64> {
    // SAFETY: sysconf is thread-safe for this constant and does not require
    // any pointer arguments.
    let ticks = unsafe { libc::sysconf(libc::_SC_CLK_TCK) };
    anyhow::ensure!(ticks > 0, "sysconf(_SC_CLK_TCK) returned {ticks}");
    Ok(ticks as u64)
}
...
fn read_boot_time_ms() -> Result<u64> {
    ...
    let btime = raw
        ...
        .context("missing btime in /proc/stat")?;
    Ok(btime.saturating_mul(1_000))
}
