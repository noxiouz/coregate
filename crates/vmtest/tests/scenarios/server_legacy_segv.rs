use vmtest::{run_scenario_from_env, server_legacy_segv_scenario};
...
fn server_legacy_segv_e2e() {
    ...
    if std::env::var_os("COREGATE_VM_KERNEL").is_none() || std::env::var_os("COREGATE_VM_INITRD").is_none() {
        eprintln!("skipping VM server_legacy_segv test: COREGATE_VM_KERNEL and COREGATE_VM_INITRD are required");
        return;
    }
